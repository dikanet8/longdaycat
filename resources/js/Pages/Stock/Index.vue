    ...
    <AppLayout>
        <div class="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
            ...
            <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div class="flex gap-1 p-1 bg-slate-100 dark:bg-white/5 rounded-2xl w-full md:w-fit">
                    <button 
                        ...
                        class="flex-1 md:flex-none px-4 md:px-6 py-2.5 rounded-xl text-xs font-bold transition-all"
                    >
                        Kelola Produk
                    </button>
                    <button 
                                    ...
                                    <tr v-for="product in filteredProducts" :key="product.id" class="hover:bg-slate-50 dark:hover:bg-white/[0.02] transition-colors group">
                                        <td class="px-8 py-5">
                                            <div class="flex items-center gap-3">
                                                <div class="w-10 h-10 rounded-xl bg-slate-100 dark:bg-slate-800 overflow-hidden shadow-inner flex-shrink-0 border border-slate-200 dark:border-white/5">
                        ...
                        <div class="md:hidden divide-y divide-slate-100 dark:divide-white/5">
                            <div v-for="product in filteredProducts" :key="'mobile-'+product.id" class="p-5 flex items-center justify-between gap-4">
                                <div class="flex items-center gap-3 min-w-0">
                                    <div class="w-12 h-12 rounded-2xl bg-slate-100 dark:bg-slate-800 overflow-hidden shadow-inner flex-shrink-0 border border-slate-200 dark:border-white/5">
                                        <img v-if="product.gambar" :src="product.gambar" class="w-full h-full object-cover" />
                                    </div>
                                    <div class="min-w-0">
                                ...
                                <div class="flex flex-col items-end gap-2 shrink-0">
                                    ...
                                    <button 
                                        @click="openUpdateModal(product)"
                                        class="px-4 py-2 bg-blue-600 text-white text-[10px] font-black uppercase tracking-wider rounded-xl shadow-lg shadow-blue-500/20 active:scale-95 transition-transform"
                                    >
                                        Update
                                    </button>
                                </div>
                    ...
                    <div class="bg-white dark:bg-slate-900/40 backdrop-blur-md rounded-3xl md:rounded-md border border-slate-200 dark:border-white/5 overflow-hidden shadow-sm">
                        ...
                        <div class="md:hidden divide-y divide-slate-100 dark:divide-white/5">
                            <div v-for="log in filteredHistory" :key="'history-mobile-'+log.id" class="p-5 space-y-4">
                                ...
                                <p v-if="log.keterangan" class="text-[11px] text-slate-500 dark:text-slate-400 font-medium bg-slate-50 dark:bg-white/5 p-3 rounded-2xl border border-slate-100 dark:border-white/5 italic">
                                    "{{ log.keterangan }}"
                                </p>
                            </div>
                        </div>
                        ...
                        <div v-if="filteredHistory.length === 0" class="px-8 py-20 text-center">
                            <div class="flex flex-col items-center">
                                <div class="w-16 h-16 bg-slate-50 dark:bg-white/5 rounded-2xl flex items-center justify-center mb-4 border border-slate-100 dark:border-white/5">
        ...
        <Teleport to="body">
            ...
            >
                <div v-if="showUpdateModal" class="fixed inset-0 z-[9999] overflow-y-auto">
                    <div class="flex items-center justify-center min-h-screen p-4 pb-32 sm:p-6 text-center">
                        <div class="fixed inset-0 bg-slate-900/80 transition-opacity" @click="showUpdateModal = false"></div>

                        <div class="relative bg-white dark:bg-slate-900 rounded-md shadow-2xl w-full max-w-sm overflow-hidden transform transition-all border border-slate-200 dark:border-white/10 text-left">
                            <div class="p-6 md:p-8">
                                <div class="flex items-center justify-between mb-6 md:mb-8">
                                    <div>
                                        <h3 class="text-xl font-black text-slate-900 dark:text-white tracking-tight uppercase">Update Stok</h3>
                                        <p class="text-slate-500 dark:text-slate-400 text-[10px] font-black uppercase tracking-widest mt-1">{{ selectedProduct?.nama_produk }}</p>
                                    </div>
                                    <button @click="showUpdateModal = false" class="p-2 bg-slate-50 dark:bg-white/5 text-slate-400 hover:text-slate-600 dark:hover:text-white rounded-md transition-all active:scale-90">
                                ...
                                <form @submit.prevent="submitUpdate" class="space-y-6">
                                    <div class="grid grid-cols-2 gap-3">
                                        <button 
                                            ...
                                            :class="[form.jenis_aktivitas === 'masuk' ? 'bg-emerald-500 text-white shadow-emerald-500/20' : 'bg-slate-50 dark:bg-white/5 text-slate-400', 'py-4 rounded-md font-black text-[10px] uppercase tracking-wider transition-all shadow-lg active:scale-95']"
                                        >
                                            Stok Masuk
                                        </button>
                                        <button 
                                            ...
                                            :class="[form.jenis_aktivitas === 'keluar' ? 'bg-rose-500 text-white shadow-rose-500/20' : 'bg-slate-50 dark:bg-white/5 text-slate-400', 'py-4 rounded-md font-black text-[10px] uppercase tracking-wider transition-all shadow-lg active:scale-95']"
                                        >
                                            Stok Keluar
                                        </button>
                                    </div>
                                    ...
                                    <div>
                                        <label class="block text-[10px] font-black uppercase tracking-widest text-slate-400 mb-2 ml-1">Jumlah</label>
                                        <input 
                                            v-model="form.jumlah"
                                            type="number" 
                                            class="w-full bg-slate-50 dark:bg-white/5 border-none rounded-md py-4 px-6 text-slate-900 dark:text-white focus:ring-2 focus:ring-blue-500 font-black text-lg ring-1 ring-slate-100 dark:ring-white/5"
                                            min="1"
                                        />
                                    </div>

                                    <div>
                                        <label class="block text-[10px] font-black uppercase tracking-widest text-slate-400 mb-2 ml-1">Keterangan</label>
                                        <textarea 
                                            v-model="form.keterangan"
                                            rows="3"
                                            placeholder="Contoh: Stok dari supplier, Barang rusak, dll."
                                            class="w-full bg-slate-50 dark:bg-white/5 border-none rounded-md py-4 px-6 text-slate-900 dark:text-white focus:ring-2 focus:ring-blue-500 text-xs font-bold ring-1 ring-slate-100 dark:ring-white/5"
                                        ></textarea>
                                    </div>
                                    ...
                                    <button 
                                        ...
                                        class="w-full bg-blue-600 text-white font-black py-5 rounded-md shadow-lg hover:scale-[1.02] active:scale-[0.98] transition-all disabled:opacity-50 text-[11px] uppercase tracking-[0.2em]"
                                    >
