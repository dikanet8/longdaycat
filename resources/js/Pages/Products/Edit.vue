...
const imagePreview = ref(props.product.gambar);

const form = useForm({
  _method:'PUT',
  kode_produk: props.product.kode_produk,
  nama_produk: props.product.nama_produk,
  kategori_id: props.product.kategori_id ||'',
  gambar: null,
  ukuran: props.product.ukuran,
  warna: props.product.warna,
  harga: props.product.harga,
  stok_minimal: props.product.stok_minimal,
});
...
const submit = () => {
  // Note: Use post with _method PUT for file uploads in Laravel
  ...
  <AppLayout>
    <div class="max-w-4xl mx-auto space-y-8 relative z-10">
      ...
      <div class="flex items-center justify-between">
        <div>
          <h1 class="text-3xl font-bold text-slate-900 dark:text-white tracking-tight">Edit Produk</h1>
          <p class="text-slate-500 dark:text-slate-400 mt-1">Perbarui informasi produk: {{ product.nama_produk }}</p>
        </div>
      ...
      <div class="bg-white dark:bg-slate-900 rounded-3xl p-8 shadow-sm dark:shadow-2xl">
        <form @submit.prevent="submit" class="space-y-8">
          ...
          <div class="flex flex-col items-center justify-center p-8 border-2 border-dashed border-slate-200 dark:border-slate-800 rounded-3xl bg-slate-50 dark:bg-white/5 hover:bg-slate-100 dark:hover:bg-slate-800/50 transition-all group relative">
            ...
            <div v-else class="text-center">
              ...
              <p class="text-slate-700 dark:text-white font-bold">Ganti Foto Produk</p>
              <p class="text-slate-500 text-xs mt-1">Biarkan kosong jika tidak ingin mengubah gambar.</p>
            </div>
            <input 
              type="file" 
