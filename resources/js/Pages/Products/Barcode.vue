    ...
    <div class="space-y-6 print:hidden">
      ...
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2 space-y-4">
          <!-- Type Toggle Buttons (Separate Container) -->
          <div class="flex justify-end w-full mb-3">
            <div class="bg-slate-50 dark:bg-slate-800 p-1.5 rounded-xl border border-slate-200 dark:border-white/5 flex items-center gap-1.5 shadow-sm">
              <button 
                ...
                :class="[barcodeType ==='CODE128' ? 'bg-blue-600 text-white shadow-md' : 'text-slate-500 hover:bg-white dark:hover:bg-slate-700']"
                class="w-24 sm:w-32 py-2 px-3 text-[10px] sm:text-xs font-black uppercase tracking-widest rounded-lg transition-all cursor-pointer text-center"
              >
                Barcode
              </button>
              <button 
                @click="barcodeType ='QR'"
                :class="[barcodeType ==='QR' ? 'bg-blue-600 text-white shadow-md' : 'text-slate-500 hover:bg-white dark:hover:bg-slate-700']"
                class="w-24 sm:w-32 py-2 px-3 text-[10px] sm:text-xs font-black uppercase tracking-widest rounded-lg transition-all cursor-pointer text-center"
              >
                QR Code
              </button>
          ...
          <!-- Toolbar (Synced Layout with Index) -->
                ...
                <tr v-for="product in filteredProducts" :key="product.id" class="hover:bg-slate-50 dark:hover:bg-white/[0.02]">
                  ...
                  <td class="px-6 py-4 hidden sm:table-cell">
                    <div class="bg-white p-2 rounded inline-block">
                      ...
                      <div v-else class="w-16 h-16 bg-white flex items-center justify-center">
                        <img :src="`https://api.qrserver.com/v1/create-qr-code/?size=100x100&data=${product.kode_produk}`" class="w-full h-full object-contain" />
                      </div>
                    </div>
                  </td>
