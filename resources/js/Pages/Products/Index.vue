    ...
    <AppLayout>
        <div class="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-700">
            ...
            <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 md:gap-6">
                ...
                <!-- Desktop Add Button -->
                <Link :href="route('products.create')" class="hidden md:flex px-6 py-3 bg-gradient-to-r from-blue-600 to-blue-700 hover:from-blue-500 hover:to-blue-600 text-white rounded-2xl shadow-lg shadow-blue-500/20 transition-all font-bold text-sm items-center gap-2 active:scale-[0.98]">
            ...
            <div class="bg-white dark:bg-slate-900/40 backdrop-blur-md p-3 md:p-2 rounded-3xl md:rounded-md border border-slate-200 dark:border-white/5 shadow-sm">
                <div class="flex flex-col md:flex-row md:items-center gap-3 md:gap-2.5">
                    ...
                    <div class="grid grid-cols-2 sm:grid-cols-4 md:flex items-center gap-2 w-full">
                        <!-- Ukuran Filter -->
                        <select v-model="filterUkuran" class="w-full md:w-40 bg-slate-50 dark:bg-white/5 border-none rounded-2xl md:rounded-md py-2.5 md:py-2 px-3 text-[11px] md:text-xs font-bold text-slate-600 dark:text-slate-400 focus:ring-1 focus:ring-blue-500 transition-all cursor-pointer ring-1 ring-slate-200 dark:ring-white/10">
                        ...
                        <!-- Warna Filter -->
                        <select v-model="filterWarna" class="w-full md:w-40 bg-slate-50 dark:bg-white/5 border-none rounded-2xl md:rounded-md py-2.5 md:py-2 px-3 text-[11px] md:text-xs font-bold text-slate-600 dark:text-slate-400 focus:ring-1 focus:ring-blue-500 transition-all cursor-pointer ring-1 ring-slate-200 dark:ring-white/10">
                            <option value="">Semua Warna</option>
                            <option v-for="warna in uniqueColors" :key="warna" :value="warna">{{ warna }}</option>
                        </select>

                        <!-- Status Stok Filter -->
                        <select v-model="filterStok" class="w-full md:w-40 bg-slate-50 dark:bg-white/5 border-none rounded-2xl md:rounded-md py-2.5 md:py-2 px-3 text-[11px] md:text-xs font-bold text-slate-600 dark:text-slate-400 focus:ring-1 focus:ring-blue-500 transition-all cursor-pointer ring-1 ring-slate-200 dark:ring-white/10">
            ...
            <div class="bg-white dark:bg-slate-900/40 backdrop-blur-md rounded-3xl overflow-hidden shadow-sm dark:shadow-2xl border border-slate-200 dark:border-white/5">
                <!-- Desktop Table -->
                            ...
                            <tr v-for="product in filteredProducts" :key="product.id" class="hover:bg-slate-50 dark:hover:bg-white/[0.02] transition-colors group">
                                <td class="px-8 py-5">
                                    <div class="flex items-center gap-4">
                                        <div class="w-12 h-12 rounded-xl bg-slate-100 dark:bg-gradient-to-br dark:from-slate-800 dark:to-slate-900 flex items-center justify-center text-slate-400 dark:text-slate-500 overflow-hidden shadow-inner flex-shrink-0">
                ...
                <div class="md:hidden divide-y divide-slate-100 dark:divide-white/5">
                    <div v-for="product in filteredProducts" :key="'mobile-'+product.id" class="p-5 flex flex-col gap-4">
                        <div class="flex items-start gap-4">
                            <div class="w-20 h-20 rounded-2xl bg-slate-100 dark:bg-gradient-to-br dark:from-slate-800 dark:to-slate-900 flex items-center justify-center text-slate-400 dark:text-slate-500 overflow-hidden shadow-inner flex-shrink-0">
                                <img v-if="product.gambar" :src="product.gambar" class="w-full h-full object-cover" />
                        ...
                        <div class="flex items-center justify-between pt-2">
                            ...
                            <div class="flex items-center gap-1">
                                <Link :href="route('products.edit', product.id)" class="p-2.5 bg-blue-50 dark:bg-blue-900/20 text-blue-600 dark:text-blue-400 rounded-xl active:scale-90 transition-transform">
                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                                    </svg>
                                </Link>
                                <button @click="confirmDelete(product)" class="p-2.5 bg-rose-50 dark:bg-rose-900/20 text-rose-600 dark:text-rose-400 rounded-xl active:scale-90 transition-transform">
                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
                                    </svg>
                                </button>
                ...
                >
                    <div v-if="showDeleteModal" class="fixed inset-0 z-[9999] overflow-y-auto">
                        <div class="flex items-center justify-center min-h-screen p-4 pb-32 sm:p-6 text-center">
                            <div class="fixed inset-0 bg-slate-900/80 transition-opacity" @click="showDeleteModal = false"></div>

                            <div class="relative bg-white dark:bg-slate-900 rounded-md shadow-2xl transform transition-all max-w-sm w-full border border-slate-200 dark:border-white/10 text-left">
                                <div class="p-8">
                                    ...
                                    <div class="text-center">
                                        <h3 class="text-2xl font-black text-slate-900 dark:text-white mb-2 tracking-tight">Hapus Produk?</h3>
                                        <p class="text-sm text-slate-500 dark:text-slate-400 font-medium">Data produk <strong>{{ productToDelete?.nama_produk }}</strong> akan dihapus permanen.</p>
                                    </div>
                                </div>
                                <div class="p-6 flex flex-col gap-3">
                                    <button @click="deleteProduct" class="w-full px-6 py-4 font-black text-white bg-red-600 rounded-md shadow-lg active:scale-95 transition-all uppercase tracking-widest text-[11px]">
                                        Ya, Hapus Sekarang
                                    </button>
                                    <button @click="showDeleteModal = false" class="w-full px-6 py-4 font-black text-slate-500 dark:text-slate-400 bg-white dark:bg-slate-800 rounded-md border border-slate-200 dark:border-white/5 active:scale-95 transition-all uppercase tracking-widest text-[11px]">
                                        Batalkan
                                    </button>
                                </div>
                            </div>
