    ...
    <AppLayout>
        <div class="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-700">
            ...
            <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 md:gap-6">
                ...
                <!-- Desktop Add Button -->
                <button @click="openCreateModal" class="hidden md:flex px-6 py-3 bg-gradient-to-r from-blue-600 to-blue-700 hover:from-blue-500 hover:to-blue-600 text-white rounded-xl shadow-lg shadow-blue-500/20 transition-all font-bold text-sm items-center gap-2 active:scale-[0.98]">
            ...
            <!-- Content section -->
            <div class="bg-white dark:bg-slate-900/40 backdrop-blur-md border border-slate-200 dark:border-white/5 rounded-3xl md:rounded-md shadow-sm overflow-hidden">
                <!-- Desktop Table -->
                            ...
                            <tr v-for="category in categories.data" :key="category.id" class="hover:bg-slate-50 dark:hover:bg-white/[0.02] transition-colors group">
                                <td class="px-6 py-4">
                                    <div class="flex items-center gap-3">
                                        <div class="w-10 h-10 rounded-xl bg-gradient-to-tr from-blue-50 to-blue-100 dark:from-blue-900/20 dark:to-blue-800/20 flex items-center justify-center border border-blue-200 dark:border-blue-800/50">
                ...
                <div class="md:hidden divide-y divide-slate-100 dark:divide-white/5">
                    <div v-for="category in categories.data" :key="'mobile-'+category.id" class="p-5 flex items-center justify-between gap-4">
                        <div class="flex items-center gap-3 min-w-0">
                            <div class="w-12 h-12 rounded-2xl bg-blue-600/10 flex items-center justify-center shrink-0 border border-blue-500/20">
                        ...
                        <div class="flex items-center gap-1 shrink-0">
                            <button @click="openEditModal(category)" class="p-2.5 bg-slate-100 dark:bg-white/5 text-slate-500 rounded-xl active:scale-90 transition-transform">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                                </svg>
                            </button>
                            <button @click="confirmDelete(category)" class="p-2.5 bg-rose-50 dark:bg-rose-500/10 text-rose-500 rounded-xl active:scale-90 transition-transform">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
                                </svg>
                            </button>
                ...
                <div v-if="categories.data.length === 0" class="px-6 py-24 text-center">
                    <div class="flex flex-col items-center justify-center animate-in fade-in zoom-in duration-500">
                        ...
                        <button @click="openCreateModal" class="px-6 py-2.5 bg-slate-900 dark:bg-white text-white dark:text-slate-900 text-xs font-bold uppercase tracking-widest rounded-xl hover:scale-105 transition-all shadow-lg">
                            Tambah Kategori
                        </button>
                    </div>
                </div>
        ...
        <Teleport to="body">
            ...
            >
                <div v-if="showModal" class="fixed inset-0 z-[9999] overflow-y-auto overflow-x-hidden">
                    <!-- Centered Container -->
                    <div class="flex items-center justify-center min-h-screen p-4 pb-32 sm:p-6 text-center">
                        <!-- Backdrop -->
                        <div class="fixed inset-0 bg-slate-900/80 transition-opacity" @click="closeModal"></div>

                        <!-- Modal Box -->
                        <div class="relative bg-white dark:bg-slate-900 rounded-md shadow-2xl w-full max-w-md overflow-hidden transform transition-all border border-slate-200 dark:border-white/10 text-left">
                            <form @submit.prevent="saveCategory">
                                <div class="px-8 py-7 border-b border-slate-100 dark:border-white/5 flex items-center justify-between bg-slate-50/50 dark:bg-white/[0.02]">
                                    <div>
                                        ...
                                        <p class="text-[10px] text-slate-500 font-bold uppercase tracking-widest mt-1">Lengkapi data berikut</p>
                                    </div>
                                    <button type="button" @click="closeModal" class="p-2 bg-white dark:bg-slate-800 text-slate-400 hover:text-slate-600 dark:hover:text-white rounded-md shadow-sm border border-slate-100 dark:border-white/5 transition-all active:scale-90">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                ...
                                <div class="p-8 space-y-8">
                                    <div class="space-y-3">
                                        <label class="block text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] ml-1">Nama Kategori</label>
                                        <div class="relative group">
                                            ...
                                            <input 
                                                ...
                                                required 
                                                class="w-full bg-slate-50 dark:bg-slate-900/80 border-none rounded-md py-4 pl-14 pr-6 text-sm font-bold text-slate-900 dark:text-white ring-1 ring-slate-200 dark:ring-white/10 focus:ring-2 focus:ring-blue-600 transition-all" 
                                                placeholder="Contoh: Makanan Kucing"
                                            >
                                        </div>
                                        <p v-if="form.errors.nama_kategori" class="text-red-500 text-[10px] font-black mt-1 ml-1 uppercase tracking-wider">{{ form.errors.nama_kategori }}</p>
                                    </div>
                                    ...
                                    <div class="space-y-3">
                                        ...
                                        <textarea 
                                            ...
                                            rows="3" 
                                            class="w-full bg-slate-50 dark:bg-slate-900/80 border-none rounded-md py-4 px-6 text-sm font-bold text-slate-900 dark:text-white ring-1 ring-slate-200 dark:ring-white/10 focus:ring-2 focus:ring-blue-600 transition-all resize-none" 
                                            placeholder="Penjelasan singkat mengenai kategori ini..."
                                        ></textarea>
                                        <p v-if="form.errors.deskripsi" class="text-red-500 text-[10px] font-black mt-1 ml-1 uppercase tracking-wider">{{ form.errors.deskripsi }}</p>
                                    </div>
                                    ...
                                    <div class="grid grid-cols-2 gap-4 pt-4">
                                        <button 
                                            type="button" 
                                            @click="closeModal" 
                                            class="px-6 py-4 bg-white dark:bg-slate-800 text-slate-500 dark:text-slate-400 font-black rounded-md border border-slate-200 dark:border-white/5 transition-all active:scale-95 uppercase tracking-widest text-[10px]"
                                        >
                                            Batal
                                        </button>
                                        <button 
                                            ...
                                            class="px-6 py-4 bg-blue-600 hover:bg-blue-700 text-white font-black rounded-md shadow-lg transition-all active:scale-95 uppercase tracking-widest text-[10px] disabled:opacity-50"
                                        >
                                            {{ form.processing ? 'Proses...' : (editMode ? 'Simpan' : 'Tambah') }}
                                        </button>
                                    </div>
                                </div>
                            </form>
